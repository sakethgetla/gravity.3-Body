...
public class movement extends Canvas implements Runnable {
	...
	int updateCounter = -1;
	int ballHeight = WIDTH/30, moonHeight = ballHeight / (sunHeight/ballHeight)  ;
	int ballWidth= ballHeight, moonWidth = ballWidth / (sunWidth/ballWidth) ;
	...
	int[] sun_xy = new int[2] , lineStartxy = new int[2], lineEndxy = new int[2], ballxy  = new int [2],
			...
			mouseMove_xy = new int[2] ,moon_xy = new int[2], moonCentre = new int[2], resetButton = new int [2] ;
	...
	private void graphics(Graphics g) {
		...
		g.fillOval(lineStartxy[0] - 3,lineStartxy[1]- 3 ,6 ,6);
		...
		g.fillRect(resetButton[0], resetButton[1], stopButtonWidth, stopButtonHeight);
		g.setColor(Color.blue);
...
;		}			
		...
		g.drawString("Reset", resetButton[0], resetButton[1]+stopButtonHeight);
	...
	private void tick() {
		...
		double line2Vel = 0.15 , lineStart2sunDist=0 ;
...
*/
	...
	private void mouseLine() {
		...
		if (mouseButtonPush){
			...
			if (lineStartxy[0] - mouseMove_xy[0] < 20 && lineStartxy[0] - mouseMove_xy[0] > -20){
				if (lineStartxy[1] - mouseMove_xy[1] < 20 && lineStartxy[1] - mouseMove_xy[1] > -20){
					p = true;
	...
	public void mousePressed(MouseEvent e) {
		...
		if (e.getButton()== MouseEvent.BUTTON1){ 
			...
			if(mousexy[0] > resetButton[0] && mousexy[1] > resetButton[1] &&
					mousexy[0] < resetButton[0] + stopButtonWidth && mousexy[1] < resetButton[1] + stopButtonHeight ){
				//new movement();
